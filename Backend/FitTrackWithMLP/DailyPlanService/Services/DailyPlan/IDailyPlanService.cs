...
namespace DailyPlanService.Services.DailyPlan
{
    public interface IDailyPlanService
    {
        ...
        Task<ReplaceDailyPlanStatus> ReplaceDailyPlanAsync(string userId, int dailyPlanId, CreateDailyPlanDto dailyPlanDto);
        Task<UpdateMealPlanStatus> UpdatePlannedMealAsync(string userId, int plannedMealId, UpdatePlannedMealDto updateDto);
    }
}
