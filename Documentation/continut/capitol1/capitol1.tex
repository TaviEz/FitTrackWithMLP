...
nonlinear constrained optimization engine with an MLP classifier to select
and scale candidate meals based on individual nutritional targets. The following 
...
\begin{itemize}
    ...
    activity level.
    \item Train and integrate an MLP classifier capable of evaluating computed
    nutritional targets to select the most appropriate candidate meals from the
    predefined meal dataset.
    \item Formulate and deploy a nonlinear constrained optimization engine using
    SLSQP to dynamically scale ingredient weights down to the gram, minimizing
    deviation from target caloric and protein boundaries.
...
stage, where an MLP classifier is trained and deployed to evaluate the
user's computed calorie and protein targets alongside the meal category,
selecting the most appropriate candidate meals from the dataset. Once
the candidate meals are selected, the pipeline transitions to a
deterministic optimization stage. Here, a nonlinear constrained
optimizer using SLSQP takes the per-meal calorie and protein targets
and mathematically adjusts the individual ingredient weights to the
gram by minimizing a weighted quadratic deviation function.

The final phase of the methodology centers on experimental evaluation.
The platform's accuracy was assessed by running six user profiles
spanning all activity groups and dietary goals through the system,
measuring the calorie deviation between the computed target and the
generated plan — consistently within 1.3\%.
...
mathematical principles behind nonlinear constrained optimization.
...
the integration between the MLP classifier and the nonlinear constrained
optimizer, and the overall software infrastructure.
