...
\begin{enumerate}
    ...
    refresh token cookie. The \texttt{UserManagementService} validates the token, queries the
    associated user, deletes the old refresh token, and persists a newly rotated one. A
    fresh JWT access token and updated refresh cookie are returned, after which the frontend
    transparently retries the original request — all without requiring the user to
    re-authenticate.
...
The per-meal \texttt{error} field is the residual reported by the linear programming
solver and reflects numerical precision rather than a meaningful deviation — values
in the range of $10^{-10}$ to $10^{-15}$ indicate that the optimization constraints
were satisfied to machine precision.
